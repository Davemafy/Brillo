...
function Login() {
  ...
  const state = useRouterState({
    select: (s) => s.location.state as HistoryState,
  });
  const router = useRouter();
  const redirect = Route.useSearch({
    select: (search: any) => search.redirect as string | undefined,
  });
  ...
  const handleLogin: SubmitEventHandler<HTMLFormElement> = async (e) => {
    ...
    setUser((user) => ({ ...(profile || data.user), isAuthenticated: true }));
      ...
      <div className="h-screen  flex flex-col  lg:flex-row gap-2">
        ...
        <div className="grow md:py-8 w-full md:mt-0 sm:max-w-md xl:p-0 lg:w-auto px-6 mx-auto flex flex-col justify-center flex-none gap-6 items-center lg:min-w-125 max-w-160 ">
          ...
          <p className="text-mediumgrey font-light">
            ...
            <Link
              to="/app/signup"
              className="inline-block ml-1 font-normal text-dark hover:text-rose"
            >
              Sign up
            </Link>
          </p>
          <p className="font-normal text-gray-700 text-center mb-2">
            ©2025
            <Link to="/" className="inline-block ml-1 text-700">
              Brillo
            </Link>
          </p>
        </div>
