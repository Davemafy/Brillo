...
function Notes() {
  ...
  const sortedNotes = [...notes].sort((current, next) => {
    if (filter === "newest") {
      ...
      return next.description.length > current.description.length ? -1 : 0;
    } else if (filter === "most popular") {
      return parseFloat(next.duration) - parseFloat(current.duration);
    }
    return 0;
  });

  const dates = notes.reduce<string[]>((all, note: Note) => {
  ...
  return (
    <>
      ...
      <div className="relative w-full h-full p-5 sm:p-8 sm:px-6 md:px-8 overflow-auto">
        {openSelectModal && (
          <SelectCoursePopUp
            courses={courses}
...
interface SelectCoursePopUpProps {
  courses: Course[];
