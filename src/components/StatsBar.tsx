      ...
      <section className='flex flex-col gap-4 mt-2 h-full overflow-auto no-scrollbar'>
        ...
        <figure className='relative text-[0.68rem] w-full flex flex-col h-full xlg:h-min overflow-auto '>
          <div className='absolute grid place-items-center inset-0 bottom-4 left-4'>
            {courses.length === 0 ? (
              <p className='text-neutral-300  font-semibold -rotate-45'>No Activity</p>
            ) : (
              <img className='mt-auto mb-3' src='/assets/img/today-graph.webp' alt='' />
            )}
          </div>
      ...
      {courses.length > 0 && (
        <article className='flex text-[0.7rem] p-3 pt-5 rounded-xl mt-auto bg-accent border border-gray-200'>
          <div className='flex flex-col gap-1'>
            ...
            <button className='mt-2 bg-dark text-[0.6rem] text-white rounded-md w-fit p-2 px-4'>Go Premium</button>
          </div>
          <div className='mx-auto'>
            <img src='/assets/img/book.webp' alt='book.webp' className={'w-26 h-auto'} />
          </div>
        </article>
      )}
    </section>
